...
> **Status dokumentu.** Powstał 2026-07-29, ostatnia aktualizacja **2026-07-29**
> (poprawka §5: komenda bramki nie wykonywała się w obrazie). Opisuje stan aplikacji
> z tej daty. Zanim go użyjesz, przejrzyj punkty oznaczone „SPRAWDŹ PRZED UŻYCIEM":
> to miejsca, które najszybciej się dezaktualizują.
>
> **Ten dokument istnieje w jednym egzemplarzu — tutaj, w repo aplikacji.** Repo platformy
> trzyma do niego odnośnik (nazwa repo + SHA), nie kopię: datowany snapshot w drugim repo
> rozjeżdża się z oryginałem i nikt tego nie zauważa. Dlatego jest wersjonowany, a nie
> lokalny — odnośnik, którego nie da się rozwinąć, jest gorszy od braku odnośnika.
...
W obrazie produkcyjnym (`docker/Dockerfile`) — i to jest komenda, którą wpisuje się
do kontraktu platformy:

```bash
python -m evals.run_retrieval_evals      # cwd MUSI być /app
```
...
> **Dlaczego dwie komendy.** W warstwie runtime obrazu **nie ma `uv`** — jest tylko
> `/app/.venv` na PATH. `evals/` jest do obrazu kopiowany, nie instalowany jako pakiet,
> więc `python -m evals.…` znajduje go wyłącznie przez katalog roboczy: Job bramki musi
> mieć `workingDir: /app`, inaczej dostaniesz `ModuleNotFoundError`. Do 2026-07-29 ta
> sekcja podawała wyłącznie wariant z `uv`, który w obrazie nie wykonywał się w ogóle —
> a `evals/` w obrazie w ogóle nie było.
...
Progi i pomiar na 2026-07-29: `recall@5` ≥ 0.938 (jest 0.958), `recall@10` ≥ 0.948
(0.969), MRR ≥ 0.850 (0.874).

> **SPRAWDŹ PRZED UŻYCIEM.** Nie przepisuj tych liczb do manifestów ani do alertów.
> Jedynym źródłem prawdy jest `evals/thresholds.yaml` w repo aplikacji — progi bywają
> podnoszone po zweryfikowanej poprawie i te wartości mogą być nieaktualne.
...
- `ui_backend.py` — tryb jednoprocesowy pod Streamlit Community Cloud, gdzie nie ma
  osobnego serwisu API. W klastrze API i UI są osobnymi deploymentami, więc ten plik
  jest tam zbędny. (Towarzyszący mu `docker/Dockerfile.spaces` został usunięty z repo
  2026-07-29 razem z porzuconą ścieżką Hugging Face Spaces.)
- `docker-compose.yml` — środowisko deweloperskie.
- `data/raw/` — korpus wgrywa się raz do bazy, nie wozi się go w obrazie.
- `requirements.txt` — istnieje wyłącznie dla Streamlit Community Cloud, który nie czyta
  `uv.lock`. Obraz produkcyjny buduje się z `uv sync --frozen`, więc w klastrze ten plik
  jest mylącym duplikatem.

---

## 9. Stan aplikacji, na którym oparty jest ten dokument

> **SPRAWDŹ PRZED UŻYCIEM.** Poniższe było prawdą 2026-07-29. Jeśli od tego czasu doszły
> zmiany w retrievalu albo w łańcuchu providerów, odśwież liczby z `evals/results/`.

| Element | Stan |
|---|---|
| Korpus | 438 chunków, 13 dokumentów |
| Model embeddingów | `intfloat/multilingual-e5-base`, CPU, in-process |
| Reranking | wyłączony (pomiar: +0.031 recall@5 za 43 s) |
| `rrf_k` | 5 |
| Model generacji | `nvidia/nemotron-nano-9b-v2:free` przez OpenRouter |
| Łańcuch fallbacku | **oba ogniwa na OpenRouterze** — brak zapasu przy awarii platformy |
| Cache odpowiedzi | w pamięci procesu, LRU + TTL, per replika |

Ostatnia pozycja jest znaną luką, nie przeoczeniem: łańcuch chroni przed wyczerpaniem
limitu i wycofaniem modelu, ale nie przed awarią samego OpenRoutera. Kandydat na drugą
platformę to Cloudflare Workers AI. Jeśli klaster ma demonstrować odporność, to jest
pierwsza rzecz do dołożenia.
